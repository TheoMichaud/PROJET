...
class StockageDonnees {
  public:
    ...
    MySQL_Connection EnvoyerDonneesLocalesBdd(MySQL_Connection connection);

    void EnregistrerDonneesSeuilsFichierLocal(String chaine);

    void OuvrirFichierZoneEcriture();

    String RecupererModeProgramme(MySQL_Connection conn);

    void EnregistrerDonneesRecupererModeProgrammeFichierLocal(String chaine);

    void OuvrirFichierModeProgramme();
int i =0;
    WiFiClient client;
  ...
  private:
    ...
    char loginUtilisateurServeur[20] = IDSERVEUR;// login de l'utilisateur du serveur
    char mdpUtilisateurServeur[15] = MDPSERVEUR;// mdp de l'utilisateur du serveur

    // Informations Wifi
    char loginWIFI[15] = IDWIFI;         //  login SSID
    char mdpWIFI[15] = MDPWIFI;     //  mot de passe SSID
